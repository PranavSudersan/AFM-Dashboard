...
def wsxm_readimg(file, header_dict, pos):
    ...
    #img data dictionary
    data_dict_chan = {'data': {'Z': ch_array*z_calib,
                               'X': xx_data.flatten(),
                               'Y':yy_data.flatten(),},
                      'header': header_dict}
        ...
        if path_ext == '.curves': # read *.curves spectroscopy files
            ...
            while True:
                ...
                data_dict_curv[curv_ind] = {'data': {'x': ch_array[::2],
                                                     'y': chan_offs+(ch_array[1::2]*chan_fact), #converted to proper units
        ...
        if path_ext == '.stp': # read *.stp spectroscopy files
            ...
            for i in range(x_num):
                ...
                if chan_label not in data_dict.keys():
                    data_dict[chan_label] = {'Curves': {}}
                if curv_ind not in data_dict[chan_label]['Curves'].keys():
                    data_dict[chan_label]['Curves'][curv_ind] = {'data': {'x': np.empty(0), 'y': np.empty(0), 'segment': np.empty(0)},
                                                                 'header': header_dict
                                                                }
                
                data_dict[chan_label]['Curves'][curv_ind]['data']['x'] = np.append(data_dict[chan_label]['Curves'][curv_ind]['data']['x'], 
                                                                           z_data)
                data_dict[chan_label]['Curves'][curv_ind]['data']['y'] = np.append(data_dict[chan_label]['Curves'][curv_ind]['data']['y'], 
                                                                           chan_offs+(ch_mat[:][i]*chan_fact))
                data_dict[chan_label]['Curves'][curv_ind]['data']['segment'] = np.append(data_dict[chan_label]['Curves'][curv_ind]['data']['segment'], 
                                                                                 y_num*[z_dir])
                # data_dict[chan_label]['Curves'][curv_ind]['header'] = header_dict #CHECK THIS. MAYBE CHANGE STRUCTURE OF DATA>separate direction
                # data_dict[chan_label]['Curves'][curv_ind] = {'data': {'x': z_data,
                #                                                       'y': chan_offs+(ch_mat[:][i]*chan_fact), #converted to proper units
                #                                                       'segment':y_num*[z_dir]},
                #                                              'header': header_dict
                #                                              }
